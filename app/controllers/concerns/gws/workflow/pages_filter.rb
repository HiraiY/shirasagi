module Gws::Workflow::PagesFilter
  ...
  def request_update
    ...
    if @item.valid?
      ...
      flash[:notice] = I18n.t("workflow.notice.request_sent")
  ...
  def restart_update
    ...
    if @item.save
      ...
      flash[:notice] = I18n.t("workflow.notice.request_restarted")
      render json: { workflow_state: @item.workflow_state }
    else
      render json: @item.errors.full_messages, status: :unprocessable_entity
    end
  ...
  def approve_update
    ...
    if params[:action] == 'pull_up_update'
      flash[:notice] = I18n.t("workflow.notice.request_pulled_up")
    else
      flash[:notice] = I18n.t("workflow.notice.request_approved")
    end
  ...
  def remand_update
    ...
    flash[:notice] = I18n.t("workflow.notice.request_remanded")
    render json: { workflow_state: @item.workflow_state }
  rescue => e
    Rails.logger.warn("#{e.class} (#{e.message}):\n  #{e.backtrace.join("\n  ")}")
    raise
  ...
  def seen_update
    ...
    flash[:notice] = I18n.t("workflow.notice.set_seen")
    render json: { workflow_state: @item.workflow_state }
  rescue => e
    Rails.logger.warn("#{e.class} (#{e.message}):\n  #{e.backtrace.join("\n  ")}")
    raise
